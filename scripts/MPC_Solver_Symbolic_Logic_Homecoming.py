...
class MPC_Solver:
    ...
    def init_mpc(self):
        ...
        model.set_rhs("r1_x", (1-collision_gain)*r1_x + (collision_gain)*800) 
        ...
        model.set_expression(
            expr_name="lagrange_term", expr= 2 * (omega**2) + 2 * (theta)**2
        )
        model.set_expression(
            expr_name="meyer_term", expr=position_error_weight * (r0_x - 640)**2
        )
        model.setup()
        print("Model defined!")
