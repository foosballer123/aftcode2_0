    ...
    def init_mpc(self):
        ...
        if collision_gain_type == 1:
            model.set_expression(expr_name="collision_gain_1",
                expr=casadi.tanh(200 * casadi.fmax(0, (self.R_F + self.R_B) - casadi.fabs(dist))))
        ...
        y_lagrange = (y_input_weight * (y_vel ** 2)
                      ...
                      + 0.5 * goal_error_weight * ((r0_y - (self.Y_MAX / 2)) * c0) ** 2 )
        y_meyer = 20 * y_position_error_weight * (y_error) ** 2
        ...
        x_lagrange = (
            ...
            + goal_error_weight * ((r0_x - 5 * self.X_MAX) * c0) ** 2
            + ball_vel_weight * ((1 / (r1_x + 0.1)) * c0) ** 2
            #+ ball_vel_weight * ((r1_x - 15) * c0) ** 2
        ...
        # ---------------------------------------------------------------
        coupling_lagrange = casadi.SX(0) + goal_error_weight / (r0_x**2 + r0_y**2 + 1.0) #+ goal_error_weight * ((dist * omega) * zm1) ** 2 #+ goal_error_weight * ( 1 / (casadi.sqrt((r0_x - 0) ** 2 + (r0_y - 0) **2 + 1e-6) + 1e-1)) ** 2
        coupling_meyer = casadi.SX(0)
