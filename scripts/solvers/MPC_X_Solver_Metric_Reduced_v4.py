...
class MPC_Solver:
    ...
    def init_mpc(self):
        ...
        if collision_gain_type == 1:
            ...
            model.set_expression(expr_name="collision_gain_5", expr=(1 / (1 + casadi.exp(-300 * ((self.R_F + self.R_B) - dist)))) * (1 / 1 + casadi.exp(-50 * (x_foot - r0_x))))
        ...
        model.set_expression(
            ...
            + ball_vel_weight * (1 / (r1_x + 1e-6))**2 
            + goal_error_weight * (r0_x - self.X_MAX)**2 
        )
        model.set_expression(
            expr_name="meyer_term", expr=
            resting_weight * (casadi.fmax(0, casadi.fabs(theta - ((15/8)*casadi.pi)) - 0.1))**2
            + goal_error_weight * (r0_x - self.X_MAX)**2 
        )
        ...
        self.mpc.bounds["lower", "_x", "theta"] = -2*casadi.pi
