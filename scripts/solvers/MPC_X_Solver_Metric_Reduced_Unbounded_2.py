...
class MPC_Solver:
    ...
    def init_mpc(self):
        ...
        model.set_expression(expr_name="c0", expr=casadi.tanh(50*casadi.fmax(0, r0_x - (self.X_ROD-0.05)))
        c0 = model.aux["c0"]
        model.set_expression(expr_name="c1", expr=casadi.tanh(50*casadi.fmax(0, r0_x - (self.X_ROD+0.05)))
        c1 = model.aux["c1"]
        ...
        model.set_expression(
            ...
            + ball_vel_weight * ( (1 / (r1_x+0.1)) * c0 * (1 - c1) )**2
            + goal_error_weight * (1 / ((r0_x - 0) + 1e-6) * c0 * (1 - c1) )**2
        )
        model.set_expression(
            expr_name="meyer_term", expr=casadi.SX(0)
            + resting_weight * (casadi.fmax(0, casadi.fabs(theta - (-(15/8)*casadi.pi)) - 0.5*casadi.pi) * c0 * c1)**2
