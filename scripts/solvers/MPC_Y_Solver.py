...
class MPC_Solver:
    ...
    def init_mpc(self, input_weight, position_error_weight, velocity_error_weight, prediction_steps, input_rate_weight):
        ...
        v_max = 31 * self.ppr  # 31 rad/s * pix/rad = pix/s           # CONVERT TO METERS (USE GLOBAL VARIABLES)
        ...
        z1 = model.aux["z1"]
        model.set_expression(expr_name="z2", expr=casadi.tanh(50*casadi.fmax(0, r0_y-120)) # CONVERT TO METERS (USE GLOBAL VARIABLES)
        ) 
        z2 = model.aux["z2"]
        model.set_expression(expr_name="z3", expr=casadi.tanh(50*casadi.fmax(0, r0_y-240)) # CONVERT TO METERS (USE GLOBAL VARIABLES)
        )
        z3 = model.aux["z3"]
        ...
        self.mpc.bounds["lower", "_x", "y1"] = 0 # CONVERT TO METERS (USE GLOBAL VARIABLES)
        self.mpc.bounds["upper", "_x", "y1"] = 120 # CONVERT TO METERS (USE GLOBAL VARIABLES)
        self.mpc.bounds["lower", "_x", "y2"] = 120 # CONVERT TO METERS (USE GLOBAL VARIABLES)
        self.mpc.bounds["upper", "_x", "y2"] = 240 # CONVERT TO METERS (USE GLOBAL VARIABLES)
        self.mpc.bounds["lower", "_x", "y3"] = 240 # CONVERT TO METERS (USE GLOBAL VARIABLES)
        self.mpc.bounds["upper", "_x", "y3"] = 360 # CONVERT TO METERS (USE GLOBAL VARIABLES) 
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.ball_flag == True:
                ...
                y1 = int(self.motor1_pos) # THIS WILL BE UPDATED IN THE MOTOR SCRIPTS TO BE METRIC
                y2 = y1 + self.P_D
                y3 = y1 + 2*self.P_D
