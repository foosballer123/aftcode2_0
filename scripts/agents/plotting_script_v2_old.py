...
#   1. Positions (linear rods)            — if linear rods present
#   2. Angles    (angular rods)           — if angular rods present
#   3. Velocities (m/s or rad/s)          — always, if any solver present
panels = []
if linear_rods:
    panels.append('linear_positions')
if angular_rods:
    panels.append('angular_positions')
if has_y_solver or has_x_solver:
    panels.append('velocities')

# Height ratios: positions panels are taller, velocity panel shorter
height_ratios = []
for p in panels:
    if p in ('linear_positions', 'angular_positions'):
        height_ratios.append(3)
    else:
        height_ratios.append(2 if args.raw_rads and has_x_solver and has_y_solver else 1)
...
fig, axes = plt.subplots(
    len(panels), 1,
    figsize=(10, 2.5 * sum(height_ratios) / max(height_ratios)),
    sharex=True,
...
# ------------------------------------------------------------
# Panel: velocities (solver commands + ball velocity)
# ------------------------------------------------------------
if 'velocities' in ax_map:
    ax = ax_map['velocities']
    ...
    if has_y_solver:
        for m in linear_rods:
            ...
            if args.raw_rads:
                ax.plot(time, omega, label=f'ω_d,y rod{m} (rad/s)', linewidth=2)
            else:
                v_mps = omega * RPP
                ax.plot(time, v_mps, label=f'Rod{m} Y Velocity (m/s)', color='red', linewidth=2)
    ...
    if has_x_solver:
        for m in angular_rods:
            ...
            omega = df[col].to_numpy()
            if args.raw_rads:
                ax.plot(time, omega, label=f'ω_d,x rod{m} (rad/s)', linewidth=2, linestyle=':')
            else:
                v_tan = omega * LP
                ax.plot(time, v_tan, label=f'Rod{m} Foot Tangential (m/s)',
                        color='darkorange', linewidth=2, linestyle=':')

    ax.set_ylabel('Velocity (rad/s)' if args.raw_rads else 'Velocity (m/s)')
    ax.axhline(0, color='black', linewidth=1, alpha=0.6)
    ax.legend(loc='upper right', fontsize=9)
    ax.grid(True, linestyle='--', alpha=0.5)
