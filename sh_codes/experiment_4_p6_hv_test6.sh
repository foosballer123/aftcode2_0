...
echo "Starting Y-Solver with --prediction_steps=6..."
python3 ~/catkin_ws/src/aftcode2_0/scripts/MPC_Y_Solver.py --prediction_steps=6 & # --> The ampersand '&' ensures that the python scripts runs as a background process so that the next line can execute
...
rosbag record /camera/color/image_raw /omega_d /ball_pos /motor1_pos -O ~/catkin_ws/src/aftcode2_0/rosbags/experiment_4_p6_hv_mode1_test6_$(date +%s) &

# Start ball simulation node and print 'mode' and 'number'
echo "Starting 'ball_sim_tests.py' with 'mode=1' and 'test=6'..."
python3 ~/catkin_ws/src/aftcode2_0/scripts/ball_sim_tests.py --mode=1 --test=6
