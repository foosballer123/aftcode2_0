...
python3 ~/catkin_ws/src/aftcode2_0/scripts/mov_agent.py --desired_pos=0
echo "Moved."

# Start MPC Solver node with desired p-horizon (using argparse)
echo "Starting Y-Solver with --prediction_steps=6 (default value)..."
python3 ~/catkin_ws/src/aftcode2_0/scripts/MPC_Y_Solver.py & # --> The ampersand '&' ensures that the python scripts runs as a background process so that the next line can execute
...
rosbag record /camera/color/image_raw /omega_d /ball_pos /motor1_pos -O ~/catkin_ws/src/aftcode2_0/rosbags/st1_p0_h6_bCR_LV_HL_$(date +"%y_%m_%d_%H_%M_%S") &

# Start ball simulation node and print 'mode' and 'number'
echo "Starting 'ball_sim_tests.py' with 'mode=1' and 'test=1'..."
python3 ~/catkin_ws/src/aftcode2_0/scripts/ball_sim_tests.py --mode=1 --test=1
